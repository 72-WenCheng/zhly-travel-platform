      ...
      <el-form :model="searchForm" class="filter-form">
        <div class="filter-row">
          <el-form-item label="民宿名称">
            <el-input v-model="searchForm.keyword" placeholder="请输入民宿名称" clearable />
          </el-form-item>
      ...
      <el-card class="batch-card-modern" v-if="selectedServices.length > 0" shadow="never">
        <div class="batch-actions-modern">
          <div class="batch-info-modern">
            ...
            <span>已选择 <strong>{{ selectedServices.length }}</strong> 个民宿</span>
          </div>
    ...
    <el-card class="table-card-modern" shadow="never">
      <div class="table-wrapper">
        ...
        >
          ...
          <el-table-column prop="title" label="民宿名称" width="200" show-overflow-tooltip>
            <template #default="{ row }">
              <span style="color: #303133; font-weight: 500;">{{ row.title }}</span>
            </template>
          </el-table-column>
          ...
          <el-table-column prop="roomType" label="房间类型" width="180" show-overflow-tooltip>
            <template #default="{ row }">
              <span style="color: #303133;">{{ row.roomType || '-' }}</span>
            </template>
          </el-table-column>
          <el-table-column prop="capacity" label="容纳人数" width="100" align="center">
            <template #default="{ row }">
              <span style="color: #303133;">{{ row.capacity || '-' }}人</span>
            </template>
          </el-table-column>
          <el-table-column prop="price" label="价格" width="120" align="center">
            <template #default="{ row }">
              <span style="color: #f56c6c; font-weight: 600;">¥{{ row.price }}/晚</span>
            </template>
          </el-table-column>
...
import { 
  getHomestayList, 
  deleteHomestay as deleteServiceApi, 
  toggleHomestayStatus as toggleServiceStatus,
  getHomestayStatistics as getServiceStatistics
} from '@/api/homestay'
import type { Homestay, HomestayListParams } from '@/api/homestay'
import { useRouter } from 'vue-router'

const router = useRouter()

// 搜索表单
const searchForm = reactive<HomestayListParams>({
  keyword: '',
  location: '',
  status: undefined
})
...
// 民宿列表
const serviceList = ref<Homestay[]>([])
const loading = ref(false)

// 选中的民宿
const selectedServices = ref<Homestay[]>([])
...
watch(
  () => [searchForm.keyword, searchForm.location, searchForm.status],
  () => {
    if (searchTimeout) {
...
const loadServiceList = async () => {
  loading.value = true
  try {
    const params: HomestayListParams = {
    ...
    const result = await getHomestayList(params)
    ...
    if (result.code === 200 && result.data) {
      ...
      serviceList.value = raw.map((item: any, idx: number) => {
        ...
        return {
          ...
          capacity: item.capacity ?? item.people ?? item.guests ?? null,
...
// 更新统计数据
const updateStats = (data: Homestay[]) => {
  serviceStats.value = {
    total: data.length,
...
// 切换状态（开启/关闭）
const toggleStatus = async (row: Homestay) => {
  try {
    const action = row.status === 0 ? '开启' : '关闭'
    await ElMessageBox.confirm(
      `确定要${action}民宿 ${row.title} 吗？`,
      '提示',
      {
...
// 设置为维护中
const setMaintenance = async (row: Homestay) => {
  try {
    await ElMessageBox.confirm(
      `确定要将民宿 ${row.title} 设置为维护中吗？维护期间用户将无法访问此民宿。`,
      '设为维护中',
      {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
...
// 恢复开放（从维护中恢复到正常）
const setNormal = async (row: Homestay) => {
  try {
    await ElMessageBox.confirm(
      `确定要恢复民宿 ${row.title} 的开放吗？恢复后用户可以正常访问。`,
      '恢复开放',
      {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
...
// 删除民宿
const deleteService = async (row: Homestay) => {
  try {
    if (row.status !== 0) {
      ElMessage.warning('只有已关闭状态的民宿才能删除！请先关闭民宿。')
      return
    }
    
    await ElMessageBox.confirm('确定要删除这个服务吗？删除后无法恢复！', '警告', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning'
    })
...
// 添加服务
const createService = () => {
  router.push('/home/admin/culture/homestays/create')
}

// 编辑民宿
const editService = (row: Homestay) => {
  router.push(`/home/admin/culture/homestays/edit/${row.id}`)
}
...
const exportServices = async () => {
  try {
    ...
    // 准备导出数据
    const exportData = dataToExport.map((service: Homestay) => ({
      'ID': service.id || '',
      '民宿名称': service.title || '',
      '位置': service.location || '',
      '联系电话': service.contactPhone || '',
      '房间类型': service.roomType || '',
      '容纳人数': service.capacity || '',
      '价格': service.price || '',
      '评分': service.rating || '',
      '浏览量': service.views || 0,
      '状态': getStatusName(service.status || 0),
      '描述': service.description || '',
      '特色': service.features ? service.features.join(',') : '',
      '设施': service.amenities ? service.amenities.join(',') : '',
    ...
    const fileName = `特色民宿数据_${dateStr}_${timeStr}_共${dataToExport.length}条.csv`
...
const batchEnable = async () => {
  try {
    await ElMessageBox.confirm('确定要启用选中的民宿吗？', '提示', {
      type: 'success'
    })
...
const batchDisable = async () => {
  try {
    await ElMessageBox.confirm('确定要禁用选中的民宿吗？', '提示', {
      type: 'warning'
    })
...
const batchDelete = async () => {
  try {
    ...
    if (notClosed.length > 0) {
      ElMessage.warning(`有 ${notClosed.length} 个民宿不是已关闭状态，无法删除！请先关闭这些民宿。`)
      return
    }
    
    await ElMessageBox.confirm('确定要删除选中的民宿吗？此操作不可恢复！', '警告', {
      type: 'warning'
    })
...
// 获取行类名
const getRowClassName = ({ row, rowIndex }: { row: Homestay; rowIndex: number }) => {
