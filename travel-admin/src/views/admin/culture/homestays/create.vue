  ...
  <div class="create-service">
    ...
    <el-card class="form-card" v-loading="loading">
      ...
      >
        ...
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="民宿名称" prop="title" required>
              <el-input v-model="formData.title" placeholder="请输入民宿名称" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
        ...
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="房间类型" prop="roomType">
              <el-input v-model="formData.roomType" placeholder="如：景观大床房 · 1室1厅1卫" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
        ...
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="价格（每晚）" prop="price" required>
              <el-input-number
        ...
        <el-form-item label="民宿描述" prop="description" required>
          <el-input
        ...
        <el-form-item label="特色标签" prop="features">
          ...
          <div class="form-tip">可输入多个特色标签，如：景观房、管家服务、双早套餐等</div>
        </el-form-item>

        <el-form-item label="亮点标签" prop="highlightTags">
          <el-select
            v-model="formData.highlightTags"
            multiple
            filterable
            allow-create
            default-first-option
            placeholder="选择或输入亮点标签"
            style="width: 100%"
          >
            <el-option
              v-for="tag in commonHighlightTags"
              :key="tag"
              :label="tag"
              :value="tag"
            />
        ...
        <el-form-item label="设施列表" prop="amenities">
          <el-select
          ...
          <div class="form-tip">如：空调、WiFi、观景阳台、早餐、停车位等</div>
        </el-form-item>
        ...
        <el-form-item label="民宿状态" prop="status" required>
...
import { 
  Plus, InfoFilled, Picture, Star, Grid, Setting
} from '@element-plus/icons-vue'
import BackButton from '@/components/BackButton.vue'
import type { FormInstance, FormRules, UploadFile, UploadFiles } from 'element-plus'
import request from '@/utils/request'
import { createHomestay, updateHomestay, getHomestayById } from '@/api/homestay'
...
const commonFeatures = [
  '景观房', '管家服务', '双早套餐', '私人影院', '温泉', 
  '观景阳台', '私密小院', '古镇风情', '传统装饰', '特色早餐'
]

const commonHighlightTags = [
  '峡谷观景', '管家服务', '私密小院', '古镇风情', '传统装饰',
  '特色早餐', '观景露台', '慢生活', '文化体验'
]

const commonAmenities = [
  '空调', 'WiFi', '观景阳台', '早餐', '停车位', '电视', 
  '热水', '吹风机', '拖鞋', '毛巾', '洗漱用品'
]

const formData = reactive<Homestay>({
  title: '',
  location: '',
  contactPhone: '',
  rating: 0,
...
const rules = reactive<FormRules>({
  title: [{ required: true, message: '请输入民宿名称', trigger: 'blur' }],
  location: [{ required: true, message: '请输入位置', trigger: 'blur' }],
  contactPhone: [{ required: true, message: '请输入联系电话', trigger: 'blur' }],
  price: [{ required: true, message: '请输入价格', trigger: 'blur' }],
  description: [{ required: true, message: '请输入民宿描述', trigger: 'blur' }],
  images: [
    { 
      required: true, 
      message: '请上传民宿图片', 
  ...
  status: [{ required: true, message: '请选择民宿状态', trigger: 'change' }]
})

const uploadUrl = computed(() => {
  const baseUrl = import.meta.env.VITE_API_BASE_URL || '/api'
  return `${baseUrl}/upload/image?folder=homestay`
})
...
const loadServiceData = async (id: number) => {
  try {
    ...
    ElMessage.error('加载民宿数据失败: ' + (error.message || '未知错误'))
    router.back()
  } finally {
    loading.value = false
  }
    ...
    if (valid) {
      ...
      try {
        ...
        if (isEdit.value && serviceId.value) {
          // 编辑模式
          result = await updateHomestay(serviceId.value, submitData)
        } else {
          // 新增模式
          result = await createHomestay(submitData)
        }
        
        if (result.code === 200) {
          ElMessage.success(isEdit.value ? '更新民宿成功' : '创建民宿成功')
          router.back()
        } else {
          ElMessage.error(result.message || (isEdit.value ? '更新民宿失败' : '创建民宿失败'))
        }
      } catch (error: any) {
