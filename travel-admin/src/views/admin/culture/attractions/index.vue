    ...
    <el-card class="table-card-modern" shadow="never">
      <div class="table-wrapper">
        ...
        >
          ...
          <el-table-column prop="ticketPrice" label="价格" width="120" align="center">
            <template #default="{ row }">
...
const exportProducts = async () => {
  try {
    ...
    // 准备导出数据
    const exportData = dataToExport.map((product: CultureAttraction) => ({
      'ID': product.id || '',
      '产品名称': product.name || '',
      '城市': product.city || '',
      '省份': product.province || '',
      '类型': getTypeName(product.type || 0),
      '地址': product.address || '',
      '价格': product.ticketPrice || '免费',
      '评分': product.rating || '',
      '浏览量': product.viewCount || 0,
      '收藏数': product.collectCount || 0,
