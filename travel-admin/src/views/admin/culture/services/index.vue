      ...
      <el-form :model="searchForm" class="filter-form">
        <div class="filter-row">
          <el-form-item label="服务名称">
            <el-input v-model="searchForm.keyword" placeholder="请输入服务名称" clearable />
          </el-form-item>
      ...
      <el-card class="batch-card-modern" v-if="selectedServices.length > 0" shadow="never">
        <div class="batch-actions-modern">
          <div class="batch-info-modern">
            ...
            <span>已选择 <strong>{{ selectedServices.length }}</strong> 个服务</span>
          </div>
    ...
    <el-card class="table-card-modern" shadow="never">
      <div class="table-wrapper">
        ...
        >
          ...
          <el-table-column prop="title" label="服务名称" width="200" show-overflow-tooltip>
            <template #default="{ row }">
...
} from '@/api/cultureService'
import type { CultureService, ServiceListParams } from '@/api/cultureService'
import { useRouter } from 'vue-router'

const router = useRouter()

// 搜索表单
const searchForm = reactive<ServiceListParams>({
  keyword: '',
  location: '',
  status: undefined
})
...
// 服务列表
const serviceList = ref<CultureService[]>([])
const loading = ref(false)

// 选中的服务
const selectedServices = ref<CultureService[]>([])
...
// 更新统计数据
const updateStats = (data: CultureService[]) => {
  serviceStats.value = {
    total: data.length,
...
// 切换状态（开启/关闭）
const toggleStatus = async (row: CultureService) => {
  try {
    const action = row.status === 0 ? '开启' : '关闭'
    await ElMessageBox.confirm(
      `确定要${action}服务 ${row.title} 吗？`,
      '提示',
      {
...
// 设置为维护中
const setMaintenance = async (row: CultureService) => {
  try {
    await ElMessageBox.confirm(
      `确定要将服务 ${row.title} 设置为维护中吗？维护期间用户将无法访问此服务。`,
      '设为维护中',
      {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
...
// 恢复开放（从维护中恢复到正常）
const setNormal = async (row: CultureService) => {
  try {
    await ElMessageBox.confirm(
      `确定要恢复服务 ${row.title} 的开放吗？恢复后用户可以正常访问。`,
      '恢复开放',
      {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
...
// 删除服务
const deleteService = async (row: CultureService) => {
  try {
    if (row.status !== 0) {
      ElMessage.warning('只有已关闭状态的服务才能删除！请先关闭服务。')
      return
    }
...
// 添加服务
const createService = () => {
  router.push('/home/admin/culture/services/create')
}

// 编辑服务
const editService = (row: CultureService) => {
  router.push(`/home/admin/culture/services/edit/${row.id}`)
}
...
const exportServices = async () => {
  try {
    ...
    // 准备导出数据
    const exportData = dataToExport.map((service: CultureService) => ({
      'ID': service.id || '',
      '服务名称': service.title || '',
      '位置': service.location || '',
      ...
      '套餐数量': service.packages ? service.packages.length : 0,
    ...
    const fileName = `农家乐服务数据_${dateStr}_${timeStr}_共${dataToExport.length}条.csv`
...
// 选择改变
const handleSelectionChange = (selection: CultureService[]) => {
...
// 获取行类名
const getRowClassName = ({ row, rowIndex }: { row: CultureService; rowIndex: number }) => {
