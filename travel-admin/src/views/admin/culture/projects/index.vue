    ...
    <div class="stats-row">
      ...
      <div class="stat-card">
        ...
        <div class="stat-content">
          ...
          <div class="stat-value">{{ serviceStats.totalInvestment }}</div>
        </div>
      </div>
      <div class="stat-card">
      ...
      <el-card class="batch-card-modern" v-if="selectedServices.length > 0" shadow="never">
        <div class="batch-actions-modern">
          <div class="batch-info-modern">
            ...
            <span>已选择 <strong>{{ selectedServices.length }}</strong> 个项目</span>
          </div>
    ...
    <el-card class="table-card-modern" shadow="never">
      <div class="table-wrapper">
        ...
        >
          ...
          <el-table-column prop="region" label="项目地区" width="150" show-overflow-tooltip>
            <template #default="{ row }">
              <span style="color: #303133;">{{ row.region }}</span>
            </template>
          </el-table-column>
          <el-table-column prop="location" label="详细位置" width="200" show-overflow-tooltip>
            <template #default="{ row }">
              <span style="color: #303133;">{{ row.location || '-' }}</span>
            </template>
          </el-table-column>
          <el-table-column prop="price" label="投资规模" width="150" align="center">
            <template #default="{ row }">
              <span style="color: #f56c6c; font-weight: 600;">¥{{ row.price }}万</span>
            </template>
          </el-table-column>
          <el-table-column prop="contactPerson" label="联系人" width="120" show-overflow-tooltip>
            <template #default="{ row }">
              <span style="color: #303133;">{{ row.contactPerson || '-' }}</span>
            </template>
          </el-table-column>
          ...
          <el-table-column prop="orderCount" label="申请数" width="100" align="center">
            <template #default="{ row }">
...
import { 
  getAdminProjectList, 
  deleteAdminProject as deleteServiceApi, 
  toggleAdminProjectStatus as toggleServiceStatus,
  getAdminProjectStatistics as getServiceStatistics
} from '@/api/cultureProject'
import type { CultureProject, ProjectListParams } from '@/api/cultureProject'
...
// 项目列表
const serviceList = ref<CultureProject[]>([])
const loading = ref(false)

// 选中的项目
const selectedServices = ref<CultureProject[]>([])
...
watch(
  () => [searchForm.keyword, searchForm.region, searchForm.status],
  () => {
    if (searchTimeout) {
...
// 切换状态（开启/关闭）
const toggleStatus = async (row: CultureProject) => {
  try {
...
// 设置为维护中
const setMaintenance = async (row: CultureProject) => {
  try {
    await ElMessageBox.confirm(
      `确定要将项目 ${row.name} 设置为维护中吗？维护期间用户将无法访问此项目。`,
      '设为维护中',
      {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
...
// 恢复开放（从维护中恢复到正常）
const setNormal = async (row: CultureProject) => {
  try {
    await ElMessageBox.confirm(
      `确定要恢复项目 ${row.name} 的开放吗？恢复后用户可以正常访问。`,
      '恢复开放',
      {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
...
// 删除项目
const deleteService = async (row: CultureProject) => {
  try {
    if (row.status !== 0) {
      ElMessage.warning('只有已关闭状态的项目才能删除！请先关闭项目。')
      return
    }
...
// 添加项目
const createService = () => {
  router.push('/home/admin/culture/projects/create')
}

// 编辑项目
const editService = (row: CultureProject) => {
  router.push(`/home/admin/culture/projects/edit/${row.id}`)
}
...
const exportServices = async () => {
  try {
    ...
    let dataToExport: CultureProject[] = []
    ...
    // 准备导出数据
    const exportData = dataToExport.map((project: CultureProject) => ({
      'ID': project.id || '',
      '项目名称': project.name || '',
      '项目地区': project.region || '',
      '详细位置': project.location || '',
      '投资规模（万元）': project.price || 0,
      '联系人': project.contactPerson || '',
      '联系电话': project.contactPhone || '',
      '评分': project.rating || '',
      '浏览量': project.viewCount || 0,
      '订单数': project.orderCount || 0,
      '状态': getStatusName(project.status || 0),
    ...
    const fileName = `政策对接数据_${dateStr}_${timeStr}_共${dataToExport.length}条.csv`
...
// 选择改变
const handleSelectionChange = (selection: CultureProject[]) => {
...
const batchEnable = async () => {
  try {
    await ElMessageBox.confirm('确定要启用选中的项目吗？', '提示', {
      type: 'success'
    })
...
const batchDisable = async () => {
  try {
    await ElMessageBox.confirm('确定要禁用选中的项目吗？', '提示', {
      type: 'warning'
    })
...
const batchDelete = async () => {
  try {
    ...
    if (notClosed.length > 0) {
      ElMessage.warning(`有 ${notClosed.length} 个项目不是已关闭状态，无法删除！请先关闭这些项目。`)
      return
    }
    
    await ElMessageBox.confirm('确定要删除选中的项目吗？此操作不可恢复！', '警告', {
      type: 'warning'
    })
...
// 获取行类名
const getRowClassName = ({ row, rowIndex }: { row: CultureProject; rowIndex: number }) => {
