  ...
  <div class="admin-list-modern">
    ...
    <div class="page-header-modern">
      ...
      <div class="header-actions">
        <el-button type="success" class="action-btn" @click="exportProducts">
          <el-icon><Download /></el-icon>
          导出数据
        </el-button>
        <el-button type="primary" class="action-btn" @click="createProduct">
    ...
    <div class="stats-row">
      ...
      <div class="stat-card">
        ...
        <div class="stat-content">
          ...
          <div class="stat-value">{{ productStats.maintenance }}</div>
        </div>
      </div>
      <div class="stat-card">
        ...
        <div class="stat-content">
          ...
          <div class="stat-value">{{ productStats.totalViews }}</div>
        </div>
      </div>
      <div class="stat-card">
        ...
        <div class="stat-content">
          ...
          <div class="stat-value">{{ productStats.closed }}</div>
        </div>
      </div>
    </div>
      ...
      <el-form :model="searchForm" class="filter-form">
        <div class="filter-row">
          ...
          <el-form-item label="标签">
            <el-input v-model="searchForm.badge" placeholder="请输入标签，如：地理标志" clearable />
          </el-form-item>
    ...
    <transition name="slide-down">
      <el-card class="batch-card-modern" v-if="selectedProducts.length > 0" shadow="never">
        <div class="batch-actions-modern">
          <div class="batch-info-modern">
            <el-icon :size="20"><Select /></el-icon>
            <span>已选择 <strong>{{ selectedProducts.length }}</strong> 个产品</span>
          </div>
          ...
          <el-table-column prop="name" label="产品名称" width="200" show-overflow-tooltip>
            <template #default="{ row }">
              <div style="display: flex; align-items: center; gap: 8px;">
                ...
                <el-tag v-if="row.certified" type="success" size="small">官方认证</el-tag>
              </div>
            </template>
          </el-table-column>
          <el-table-column prop="origin" label="产地" width="150" show-overflow-tooltip>
            <template #default="{ row }">
              <span style="color: #303133;">{{ row.origin || row.city || '-' }}</span>
            </template>
          </el-table-column>
          <el-table-column prop="price" label="价格" width="120" align="center">
            <template #default="{ row }">
              <div v-if="row.price || row.ticketPrice">
                <span style="color: #f56c6c; font-weight: 600;">
                  ¥{{ row.price || row.ticketPrice }}
                </span>
                <span v-if="row.unit" style="color: #909399; font-size: 12px;">{{ row.unit }}</span>
              </div>
          ...
          <el-table-column label="操作" width="320" fixed="right" align="center">
            <template #default="{ row }">
              <div class="action-buttons">
                <el-button type="info" size="small" text @click="editProduct(row)">
...
import { 
  ShoppingBag, Download, Plus, Search, Refresh, Check, Warning, 
  Edit, Lock, Unlock, Delete, Select, CloseBold, Close, Tools, View, ArrowLeft, ArrowRight
} from '@element-plus/icons-vue'
import { formatDateTime } from '@/utils'
import { 
  getAdminAttractionList, 
  deleteAdminAttraction, 
  toggleAdminAttractionStatus,
  getAdminAttractionStatistics
} from '@/api/cultureAttraction'
import type { CultureAttraction, AttractionListParams } from '@/api/cultureAttraction'
...
// 特色周边列表
const productList = ref<CultureAttraction[]>([])
const loading = ref(false)

// 选中的特色周边
const selectedProducts = ref<CultureAttraction[]>([])
...
// 获取图片列表
const getImageList = (row: CultureAttraction): string[] => {
...
watch(
  () => [searchForm.keyword, searchForm.origin, searchForm.badge, searchForm.status],
  () => {
    if (searchTimeout) {
...
const loadProductList = async () => {
  ...
  try {
    ...
    if (searchForm.origin) {
      params.city = searchForm.origin
    }
    
    if (searchForm.badge) {
      params.keyword = searchForm.badge
    }
...
// 更新统计数据
const updateStats = (data: CultureAttraction[]) => {
  productStats.value = {
    total: data.length,
...
const loadStats = async () => {
  try {
    const result = await getAdminAttractionStatistics()
    if (result.code === 200 && result.data) {
      productStats.value = {
...
// 切换状态（开启/关闭）
const toggleStatus = async (row: CultureAttraction) => {
  try {
    ...
    const result = await toggleAdminAttractionStatus(row.id!, newStatus)
...
// 设置为维护中
const setMaintenance = async (row: CultureAttraction) => {
  try {
    await ElMessageBox.confirm(
      `确定要将产品 ${row.name} 设置为维护中吗？维护期间用户将无法访问此产品。`,
      '设为维护中',
      {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
    ...
    const result = await toggleAdminAttractionStatus(row.id!, 2)
...
// 恢复开放（从维护中恢复到正常）
const setNormal = async (row: CultureAttraction) => {
  try {
    await ElMessageBox.confirm(
      `确定要恢复产品 ${row.name} 的开放吗？恢复后用户可以正常访问。`,
      '恢复开放',
      {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
    ...
    const result = await toggleAdminAttractionStatus(row.id!, 1)
...
// 删除产品
const deleteProduct = async (row: CultureAttraction) => {
  try {
    if (row.status !== 0) {
    ...
    const result = await deleteAdminAttraction(row.id!)
...
// 添加产品
const createProduct = () => {
  router.push('/home/admin/culture/souvenirs/create')
}

// 编辑产品
const editProduct = (row: CultureAttraction) => {
  router.push(`/home/admin/culture/souvenirs/edit/${row.id}`)
}

// 导出产品
const exportProducts = async () => {
  try {
    const totalCount = pagination.total || productStats.value.total || 0
    
    await ElMessageBox.confirm(
      `确定要导出全部产品数据吗？预计导出 ${totalCount} 条记录`,
      '导出确认',
      {
    ...
    let dataToExport: CultureAttraction[] = []
    ...
    try {
      ...
      if (searchForm.origin) params.city = searchForm.origin
      if (searchForm.badge) params.keyword = searchForm.badge
    ...
    const exportData = dataToExport.map((product: any) => ({
      ...
      '产品名称': product.name || '',
      '标签': product.badge || '',
      '官方认证': product.certified ? '是' : '否',
      '产地': product.origin || product.city || '',
      '价格': product.price || product.ticketPrice || 0,
      '单位': product.unit || '',
      '销量': product.sales || 0,
      '评分': product.rating || '',
      '浏览量': product.viewCount || 0,
    ...
    const fileName = `特色周边数据_${dateStr}_${timeStr}_共${dataToExport.length}条.csv`
...
// 选择改变
const handleSelectionChange = (selection: CultureAttraction[]) => {
...
const batchDelete = async () => {
  try {
    const notClosed = selectedProducts.value.filter(a => a.status !== 0)
...
// 获取行类名
const getRowClassName = ({ row, rowIndex }: { row: CultureAttraction; rowIndex: number }) => {
